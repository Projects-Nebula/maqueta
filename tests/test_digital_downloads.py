...
def test_success_page_falls_back_to_provider_lookup_before_webhook_lands(anon_api, user):
    ...
    assert not Order.objects.filter(stripe_session_id=session.id).exists()

    response = anon_api.get(f"/gracias/?session_id={session.id}")
    assert response.status_code == 200
